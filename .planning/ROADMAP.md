...
# Milestone v1.3 — 추출→공유→투표 자연 흐름 (iOS 협업 동선)

**Created:** 2026-06-14
**Milestone:** v1.3
**Core Value:** iOS에서 링크 추출 직후 장소가 보이고, 한 번에 친구와 공유해 web에서 투표까지 자연스럽게 이어지는 동선 완성 (v1.1 web 투표를 iOS가 비로소 사용 가능하게 연결)
**Granularity:** standard → **1 phase** (단일 응집 기능, wave별 4 plan)
**Design source:** [docs/superpowers/specs/2026-06-14-extract-to-vote-flow-design.md](../../docs/superpowers/specs/2026-06-14-extract-to-vote-flow-design.md) (brainstorming 승인)

> v1.0(1~7)/v1.1(8~10)/v1.2(11~13) 위에 보존. 번호는 Phase **14**부터 이어간다.

## v1.3 결정 근거 요약 (브레인스토밍 잠금)

고객 재점검 결과: 추출은 되지만 직후가 비어 있음 — ① 장소 리스트 UI 없음(링크 목록만), ② 지도가 `initialRegion` 1회성으로 도쿄 고정(`fitToCoordinates` 없음), ③ 이 보드 공유/투표 진입점 없음. 투표 경험은 web `/b/[slug]`에 이미 완성. 빠진 건 *iOS에서 보여주고 공유로 잇는 다리*.

**사용자 잠금 결정 (5):**
- **공유 순간:** "친구와 정하기" 가이드 CTA + 상단 상시 공유 아이콘 (둘 다).
- **큐레이션:** 다듬지 않고 바로 공유 — 투표(❤️)가 우선순위를 만든다.
- **레이아웃:** A안(리스트 중심) — 지도 띠(핀 자동맞춤) + CTA + flat 장소 리스트.
- **여러 영상:** flat + 장소별 출처 ▶칩, place id로 dedup(투표 모임). 2뎁스 그룹 미채택(투표 분산).
- **장소 ▶:** 앱 안 임베드 플레이어가 `source_timestamp_sec`부터 재생(B안).

## Phases (v1.3)

- [~] **Phase 14: 추출→공유→투표 자연 흐름** — iOS 보드 flat 장소 리스트 + 지도 fitToCoordinates + 장소 영상 인앱 임베드 재생 + "친구와 정하기"/상시 공유 → 네이티브 공유. web 투표화면 재사용. (code-complete 4/4 + GHRV 픽스 · 14-02 라이브 검증 · share/video tap-through UAT 잔여)

---

## Phase Details (v1.3)

### Phase 14: 추출→공유→투표 자연 흐름

**Goal:** iOS 사용자가 링크를 추출하면 장소가 리스트와 지도(자동맞춤)로 즉시 보이고, 장소의 ▶로 영상의 해당 장면을 앱 안에서 보며, "친구와 정하기"(또는 상시 공유)로 보드를 shared 전환하고 `${webUrl}/b/{slug}`를 공유해 친구가 web에서 ❤️ 투표할 수 있다.
**Depends on:** v1.1 Phase 10 (web 투표·`joinSharedBoard`·`vote-island` — 재사용), v1.2 (SDK 56 빌드 — react-native-webview 추가가 그 위에 올라감)
**Why now:** v1.1에서 web 투표는 됐지만 iOS가 그 보드를 보여주고 공유하는 다리가 없어 dogfooding의 공유 면이 안 굴러감.
**Success Criteria** (what must be TRUE):
  1. 추출 완료 후 보드 화면에 장소가 **리스트로** 보인다(링크 목록이 아니라 장소).
  2. 장소가 1개 이상이면 지도가 **모든 핀을 담도록 자동 맞춤**된다(도쿄 고정 아님).
  3. 여러 영상을 추가해도 리스트는 flat이고 같은 place id는 **한 줄**로 합쳐진다.
  4. 장소 ▶를 누르면 **앱 안에서** 그 영상이 `source_timestamp_sec`부터 재생된다(유튜브+타임스탬프).
  5. "친구와 정하기"(또는 상시 공유) 탭 → 보드 shared 전환 + `${webUrl}/b/{slug}` 공유 시트.
  6. 그 URL을 비로그인으로 열면 web 투표 화면이 나오고 ❤️ 투표가 된다(기존 회귀 0).
**Plans:** 4 plans (code-complete 4/4 · 14-02 라이브 검증 · share/video tap-through UAT 잔여)
  - [x] 14-01-PLAN.md — 백엔드: `shareBoard()` api 래퍼 [Wave 1] (299033d) **DEVIATION: 마이그레이션 불필요** — 0001에 `boards_share_slug_before_update` 트리거가 이미 있어 visibility→shared UPDATE가 slug 자동생성. RPC·db push 게이트 제거.
  - [x] 14-02-PLAN.md — iOS flat 장소 리스트(출처 ▶칩) + 지도 `fitToCoordinates` [Wave 2] (7b84289) **✅ 라이브 검증**: 19개 장소 리스트 + 지도가 삿포로 핀에 자동맞춤.
  - [x] 14-03-PLAN.md — iOS 장소 영상 인앱 임베드(`react-native-webview`, PinBottomSheet) [Wave 2] (97e4e2a) + GHRV 루트 래핑 픽스(9f29c6c). 빌드·webview pod OK. **시트/플레이어 런타임 시각 검증 잔여**(시뮬레이터 macOS 알림센터 오버레이가 클릭 차단).
  - [x] 14-04-PLAN.md — iOS "친구와 정하기" CTA + 상시 공유 → `shareBoard` → 네이티브 Share [Wave 3] (4b803aa) **✅ UI 라이브**: 공유 버튼 + CTA 표시. **공유 시트 트리거 tap-through UAT 잔여.**
**UI hint:** yes
**라이브 빌드 검증(2026-06-14):** SDK 56 + Google Maps + webview 빌드 성공. 신규 보드에 삿포로 영상 추출 → **19개 장소** + flat 리스트 + 지도 fit + 공유/CTA 표시 확인. GestureHandlerRootView 루트 누락(기존 버그) 발견·수정. 잔여: 장소 탭→영상 플레이어 + 공유 시트 시각 확인(알림센터 오버레이로 차단).
**비범위(후속):** iOS 내 투표결과/확정 보기 · 큐레이션/삭제 UI · 영상별 그룹 필터 · cross-source place id 정규화.

### Phase 15: 장소 카테고리 보강 (LLM vibe) — 추출 LLM에 vibe 필드 + 맵링크 vibe 매칭 + core 단일 카테고리 resolver 통일
...
*v1.3 Roadmap created: 2026-06-14 (brainstorming spec 기반)*
*Next: `/gsd-execute-phase 14` (또는 plan 검토 후 실행)*
