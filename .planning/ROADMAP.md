...
**Plans:** 4 plans
  ...
  - [ ] 01-02-PLAN.md — app.config.ts wire-up (icon/splash/expo-font) + NativeWind smoke screen + Path A 실기기 빌드 + SESSION-NOTES timeline [BUILD-01/02/03]
  - [~] 01-03-PLAN.md — next/font/local Pretendard + lib/env.ts + dev-tool 이중 게이트 (page redirect + component null) [WEB-01/02 + BUILD-03 web] (Tasks 1+2 done 2026-05-25, Task 3 build+curl verify pending — see 01-03-SUMMARY.md)
  - [ ] 01-04-PLAN.md — (조건부) EAS Build development profile fallback — 02-PLAN Path A가 4h timebox 도달 시에만 [BUILD-01/03]
**UI hint:** yes
...
**Plans:** 3 plans
  - [ ] 02-01-PLAN.md — Migration 0004 (extraction_costs table + places columns) + shared constants + FieldMask verification [EXTRACT-03/04/05]
  - [ ] 02-02-PLAN.md — Edge Function hardening: broadcast, citation filter, cost logging, source_kind/inferred_city wiring [EXTRACT-01/02/04]
  - [ ] 02-03-PLAN.md — Schema push + type regeneration + GCP billing alert setup [EXTRACT-06]
...
|-------|----------------|--------|-----------|
| 1. Build Unblock & Hygiene | 1/4 (+1 partial) | In progress (handoff to teammate) | - |
| 2. Extraction Pipeline Hardening | 0/3 | Planned | - |
