...
**Plans:** 5 plans
  - [ ] 03-01-PLAN.md — Migration 0005 (extraction_costs.link_id nullable) + packages/core APP_GROUP_ID + SharedDefaultsKeys + extractChannelName + jest-expo test infra + docs/manual-uat-phase3.md [SAVE-04, SAVE-05]
  - [ ] 03-02-PLAN.md — expo-share-intent@^6.1.1 + app.config.ts plugin + App Group entitlement + EAS appExtensions + prebuild + real-device share-sheet smoke checkpoint [SAVE-03, SAVE-04]
  - [ ] 03-03-PLAN.md — resolve-place Edge Function (FieldMask 5 fields, max 5 results, extraction_costs link_id=null) + Zod ResolvePlace schemas + renamePlace/deletePlace helpers [SAVE-05]
  - [ ] 03-04-PLAN.md — SharedDefaults Swift module + lib/pending.ts drainPendingLinks (D-04/D-05/D-06) + AppState wiring in _layout + auth gate restore (index.tsx/login.tsx) + failed-banner in boards.tsx + Toast host + realtime helper [SAVE-01, SAVE-04]
  - [ ] 03-05-PLAN.md — @gorhom/bottom-sheet@^5.2.14 + boards/[id].tsx integration (broadcast subscribe + spinner overlay + + 핀 button) + PinBottomSheet + PinAddModal + real-device UAT execution [SAVE-01, SAVE-02, SAVE-03, SAVE-04, SAVE-05]
**UI hint:** yes
