...
- [x] 27-02-PLAN.md — NAME-01 가고싶어→찜 카피 스윕 + WORKSTREAMS·ARCHITECTURE 역할 기술 v2.1 갱신
- [x] 27-03-PLAN.md — 배포(main push + EF --use-api) + 27-HUMAN-UAT.md 통합 체크리스트 + 라이브 게이트 무비용 실증
...
1. 대상은 `apps/web`의 `/onboarding` (배포처 moajoa-web.vercel.app). **iOS는 v2.1 동결 — 수정 금지** (`apps/ios/app/trip/[id]/(tabs)/plan.tsx`는 읽기 전용 analog)
...
9. 링크 유무 분기: 유튜브 링크가 있어 장소 추출이 가능하면 AI 일정을 추천. 링크가 없으면 모아만 생성하고, 이후 링크 추가 시 추출 → 일정 추천. 장소 검색으로 추가할 때는 **"몇 일차에 넣을지" 묻고** 해당 Day에 배치, "아직 모르겠다"면 AI 배치에 맡기고 나중에 변경 가능. **이 규칙은 사용자에게 가이드 카피로 노출**

**Open gray areas** (discuss에서 잠글 것):

- 플랜 미생성 상태에서는 Day가 존재하지 않는다(`plan_items`는 플랜의 자식) — 장소 검색 추가 시 "몇 일차?"를 어떻게 처리할지: 질문 숨김 / `day_count` 기준 Day 후보 선노출 후 플랜 생성 시 반영 / unplaced pool
- 날짜·기간 모두 미정인 모아의 결과 화면·Day 탭 처리
- 추출이 비동기라 링크 추가 후 장소가 늦게 생김 — 일정 추천 트리거 시점·진행 표시

**Success Criteria** (what must be TRUE):

1. `/onboarding` 스텝들이 레퍼런스 IMG_2918~2921과 동일한 레이아웃(뒤로가기 + `N/총` 카운터, 중앙 아이콘, 큰 타이틀 + 서브카피, 2열 pill, 하단 고정 CTA)으로 렌더된다
2. 날짜 스텝에서 기간 pill을 고르면 `trips.day_count`에 저장되고, 캘린더 버튼으로 정확한 날짜 범위도 선택할 수 있다
3. 링크 붙여넣기·장소 검색이 위저드에서 기존과 동일하게 동작한다 (`AddContentTabs` 재사용 — 병렬 구현 0)
4. 유튜브 링크로 장소가 추출된 모아에서 '일정 만들기'를 누르면 Day 1~N 탭 + 번호 타임라인 결과 화면이 뜬다 (날짜 미정이어도 `day_count` 기준 Day 수)
...
- [x] 28-03-PLAN.md — D-21 수동배치 Day 고정 계약 + EF day_count fallback (SC-4·SC-5)
- [x] 28-04-PLAN.md — 위저드 리스타일: 헤더·타이틀·2열 pill·CTA + 기간 pill 스텝 + build-draft (SC-1·SC-2·SC-3)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 28-05-PLAN.md — [일정] 결과 화면: PlanSection · DurationGateSheet · DaySelectSheet (SC-4·SC-5)
...
**Requirements**: 신규 (discuss/plan에서 CHAT-04 계열 발급 — REQUIREMENTS.md 미매핑, 아래 Success Criteria를 requirement 축으로 사용) · **CHAT-08** (호스트 투표 현황 가시성 — UAT Gap 3, 2026-07-14 발급) · **CHAT-09** (게스트 /t 채팅 진입 어포던스 — UAT new_request, 2026-07-15 발급) · **CHAT-10** (게스트 /t join 전 실제 채팅 메시지 열람 — UAT new_request, 2026-07-15 발급)
...
**Success Criteria** (what must be TRUE — discuss에서 확정·정제):
  ...
  7. **(CHAT-10)** 게스트가 /t(places·dates·both) join 전에도 실제 채팅 메시지 스냅샷을 읽는다 (empty-state 대신 — anon-grant DEFINER RPC 0034). 입력 시도 시 게이트는 CHAT-09 그대로 (UAT new_request)
...
**Plans:** 4/4 plans executed + 3 gap-closure plans
...
- [x] 29-07-PLAN.md — [gap-closure] UAT new_request (CHAT-10): 게스트 /t join 전 실제 채팅 메시지 열람 — 신규 0034 `public_trip_messages` anon-grant DEFINER 읽기 RPC(public_trip_poll 0029 미러·user_id PII 미노출·limit 200) + `getPublicTripMessages` 래퍼 + guest-surface teaser empty-state→스냅샷 렌더(read-only, 29-06 입력 게이트 무회귀). 비멤버 realtime 없음(1회 fetch, 라이브는 join 후). — ✅ 2026-07-15 (api 113·web 343/343·tsc 0·build PASS ƒ /t/[slug] 4.25kB·migrations diff=0034만·iOS/core/moa diff 0·deviation 0, commits d44ec49·67634cc·6871764·77ac311·9968ba1; 원격 0034 push resolved — origin/main d44ec49..5c5a21c fast-forward, Supabase↔GitHub 적용, 라이브 스팟 체크는 verify-work) — `29-07-SUMMARY.md`

Waves: W1 = 29-01 ∥ 29-02 → W2 = 29-03 → W3 = 29-04 · [gap] 29-05 · [gap] 29-06 · [gap] 29-07 (각 독립)
