...
**Plans:** 6 plans (5 waves — Wave 1 foundation → Wave 2 parallel UI [iOS step indicator ∥ Web marker] → Wave 3 iOS retry → Wave 4 iOS low-conf sheet → Wave 5 iOS onboarding card)
  - [ ] 05-01-PLAN.md — Migration 0006 (places.confidence + public_board_view RPC redef + profiles_create_first_board trigger + backfill) + Edge Function confidence wire + @moajoa/core constants (EXTRACT_STEP_KO / LOW_CONFIDENCE_THRESHOLD / OnboardKeys) + @moajoa/api confirmAiPlace/rejectAiPlace helpers + types regen [TRUST-01, TRUST-04, ONBOARD-01]
  - [ ] 05-02-PLAN.md — iOS StepIndicator component + [id].tsx broadcast subscribe 5-stage wire (TRUST-02) [Wave 2 ∥ 05-05]
  - [ ] 05-03-PLAN.md — Toast action slot + error 8s + [id].tsx retry on broadcast 'error' + link list row status with failed-tap retry (TRUST-03) [Wave 3 — sequential w.r.t. [id].tsx]
  - [ ] 05-04-PLAN.md — iOS marker source_kind+confidence visual + PinBottomSheet low_confidence variant ([확인]/[잘못됨]) (TRUST-01 iOS + TRUST-04) [Wave 4]
  - [ ] 05-05-PLAN.md — Web public-board-map.tsx Marker SVG color/opacity + ? badge (TRUST-01 web parity) [Wave 2 ∥ 05-02]
  - [ ] 05-06-PLAN.md — lib/onboarding.ts AsyncStorage wrapper + OnboardCard component + [id].tsx visibility wire (ONBOARD-02) [Wave 5]
**UI hint:** yes
...
| 5. Trust UI & Onboarding | 0/6 | Planned     | - |
