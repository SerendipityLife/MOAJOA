...
progress:
  ...
  partial_plans: 0
  percent: 86
stopped_at: Phase 5 Wave 3 완료 — 05-03 ✓ 2026-05-26 (~3분, 3 commits f7c7938 + aa62d8c + fc2dad1). apps/ios/lib/toast.tsx 시그니처 확장 — showToast(message, kind, options?: { durationMs?, action? }) backward-compatible 옵션 객체 (grep 결과 numeric 3rd-arg 호출자 0). error kind 기본 duration 5000→8000ms (D-10 retry window). ToastHost는 current.action 있으면 우측 underline Pressable 액션 버튼 + flex-row layout + 8px gap; action onPress가 hideToast() wrap (caller가 lifecycle 안 만짐). apps/ios/__tests__/toast.test.tsx 신규 6 cases (action 가시·onPress 호출·Phase 3 회귀·error 8s default·info 3s default·explicit override; SafeAreaProvider initialMetrics wrapper로 useSafeAreaInsets jest 호환). [id].tsx broadcast 'error' 분기는 linkId를 setAnalyzing(null) 전에 capture → { label: '재시도', onPress } 전달; retry는 setAnalyzing(linkId) 재진입 + triggerExtraction 재호출 + nested catch '재시도 실패' terminal toast (D-12 user-explicit only). onAddLink catch 분기도 동일 retry 패턴 mirror (consistent failure surface). renderItem 확장 — extraction_status 5-key 한국어 fixture (pending/processing/ready/failed/manual_review → 분석 대기/분석 중.../분석 완료/분석 실패 — 탭하여 재시도/재추출 필요) inline (D-11 web parity 없음 — iOS only D-17 lock); failed 행만 Pressable enabled, 다른 status disabled={!isFailed} no-op; failed status 단어만 text-danger 색, 나머지 text-neutral-500. text-xs 사이즈 유지 (D-22 lock 활성 use site에 링크 리스트 row 없음 — Karpathy §3.3 surgical). triggerExtraction wire 3 retry sites (broadcast / onAddLink / row) + 1 original = 4 calls. 20/20 jest (4 suites — toast.test.tsx 6 신규 + step-indicator/realtime/pending 회귀 0), typecheck clean. Plan §"Verification Loop" live device UAT는 end-of-phase batch deferred per auto mode. 이전: Phase 5 Wave 2 완료 — 05-05 ✓ 2026-05-26 (~4분, 2 commits 8e35322 + 2464a12). apps/web/lib/marker-svg.ts 신규 pure buildMarkerIconUrl (source_kind+confidence → SVG data URL, AI #F97316 + manual #0F172A, low-conf opacity 0.45 + 백색 ? 14px sans-serif, undefined/null=high-conf safe fallback per D-15). apps/web/__tests__/marker-svg.test.ts 8 케이스 (matrix branches + strict < 0.7 + SVG geometry) — PASS. apps/web/app/b/[slug]/_components/public-board-map.tsx +13 lines (icon: { url, scaledSize g.Size(32,40), anchor g.Point(16,40) }; 클릭 핸들러 D-14 unchanged). packages/core/src/schemas/place.ts +21 lines (PlaceSchema source_kind z.enum(['ai','manual']) + confidence z.number().min(0).max(1).nullable()). packages/core/src/types/index.ts (PublicBoardView.places Pick 확장 source_kind+confidence). map-options.test.ts: Rule 3 inline fix — g.Size+g.Point stubs + 픽스처에 source_kind+confidence 필드 추가 (component contract bump in same commit). 49/49 web vitest PASS, typecheck clean, build green (/b/[slug] 2.76kB / 118kB). 이전: 05-02 ✓ 2026-05-26 (~6분, 2 commits 3aac855 + 3c2c113). apps/ios/app/boards/_step-indicator.tsx 신규 (58 lines, EXTRACT_STEP_KO 4-키 import, D-22 visual reassignment current=text-base/font-semibold/brand-500 + ● / done=text-sm/text-neutral-500 + ● / future=text-xs/font-medium/text-neutral-300 + ○, ActivityIndicator 상단 항상 spin), Step union export ('metadata'|'transcript'|'llm'|'places'). [id].tsx surgical extend: currentStep state + subscribeExtractProgress 콜백 분기 + overlay 본문 교체. __tests__/step-indicator.test.tsx 5 cases — 전체 14/14 PASS, typecheck clean. 이전: Phase 5 Wave 1 ✓ — 05-01 완료 2026-05-26 (~4분, 4 commits ef8e842 + c206097 + 5d3f194 + 3732511; T2 type regen deferred — Docker daemon not running + no supabase access token). Migration 0006_trust_ui_onboarding.sql 129 lines (4 parts atomic: places.confidence numeric(3,2) CHECK 0..1 + public_board_view RPC redef append source_kind+confidence + profiles_create_first_board AFTER INSERT trigger SECURITY DEFINER idempotent NEW.id guard + backfill SELECT existing profiles → '내 첫 여행' visibility=private). User actions deferred: (1) `supabase db push` for migration 0006 + (2) `pnpm supabase:types` after push + (3) backfill verify SQL. (~6분, 2 commits 3aac855 + 3c2c113). apps/ios/app/boards/_step-indicator.tsx 신규 (58 lines, EXTRACT_STEP_KO 4-키 import, D-22 visual reassignment current=text-base/font-semibold/brand-500 + ● / done=text-sm/text-neutral-500 + ● / future=text-xs/font-medium/text-neutral-300 + ○, ActivityIndicator 상단 항상 spin), Step union export ('metadata'|'transcript'|'llm'|'places'). [id].tsx surgical extend: currentStep state + subscribeExtractProgress 콜백 분기 (done/error에 setCurrentStep(null) cleanup 추가, metadata|transcript|llm|places → setCurrentStep(p.step)) + overlay 본문 교체 (ActivityIndicator+Text → <StepIndicator current={currentStep}/>) + 사용 안하는 ActivityIndicator import 제거 (Karpathy §3.3). __tests__/step-indicator.test.tsx 5 cases (null state + current at each position + className-substring 토큰 검증 robust to NativeWind ordering) — 전체 14/14 PASS, typecheck clean. Live device UAT는 jest visual contract로 covered하고 end-of-phase batch로 deferred. 이전: Phase 5 Wave 1 ✓ — 05-01 완료 2026-05-26 (~4분, 4 commits ef8e842 + c206097 + 5d3f194 + 3732511; T2 type regen deferred — Docker daemon not running + no supabase access token). Migration 0006_trust_ui_onboarding.sql 129 lines (4 parts atomic: places.confidence numeric(3,2) CHECK 0..1 + public_board_view RPC redef append source_kind+confidence + profiles_create_first_board AFTER INSERT trigger SECURITY DEFINER idempotent NEW.id guard + backfill SELECT existing profiles → '내 첫 여행' visibility=private). extract-youtube/index.ts +1 line in places upsert (confidence: r.cand.confidence, Zod default 0.5 already in pipeline/claude.ts). packages/core/src/constants.ts +31 lines EXTRACT_STEP_KO (D-09 한국어 fixture) + LOW_CONFIDENCE_THRESHOLD=0.7 (D-15) + OnboardKeys.LinkCardDismissed (D-20). packages/api/src/queries/places.ts +31 lines confirmAiPlace (UPDATE source_kind=manual confidence=null, D-04/D-14) + rejectAiPlace alias of hidePlace (mirror Phase 3 deletePlace alias pattern). Rule 1 fix: T2's `pnpm supabase:types` shell-redirect (`> database.ts`) zeroed pre-existing 1545-line types when `supabase gen --local` failed (Docker not running); restored via `git checkout HEAD -- packages/api/src/types/database.ts`. User actions deferred: (1) `supabase db push` for migration 0006 + (2) `pnpm supabase:types` after push for confidence/source_kind type narrowing in Plans 05-02~05-06 + (3) backfill verify SQL. 05-02~05-06 코드는 build/typecheck pass (Phase 3 `as Place` cast pattern works until regen).
---
...
- **Next action:** Phase 5 Wave 4 — 05-04 (iOS marker visual + PinBottomSheet low-conf [확인]/[잘못됨]; sequential w.r.t. [id].tsx after 05-03). 그 후 Wave 5 (05-06 onboarding card).
- **Progress:** [██████████░] Overall 19/22 plans (Phase 1 + 2 완료 + Phase 3 5/5 + Phase 4 4/4 code-complete + Phase 5 4/6; UAT-pending for Phase 3 + 4 + 5-02 + 5-03 + 5-05). 86%.
...
| 5 | Trust UI & Onboarding | TRUST-01..04, ONBOARD-01..02 (6) | **Current (Wave 1+2 done — 3/6)** |
