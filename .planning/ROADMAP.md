...
- [ ] **Phase 14: 추출→공유→투표 자연 흐름** — iOS 보드 flat 장소 리스트 + 지도 fitToCoordinates + 장소 영상 인앱 임베드 재생 + "친구와 정하기"/상시 공유 → `share_board` RPC → 네이티브 공유. web 투표화면 재사용.
...
**Plans:** 4 plans
  - [ ] 14-01-PLAN.md — 백엔드: 마이그레이션 `0014_share_board_rpc.sql` (SECURITY DEFINER `share_board`) + `packages/api` `shareBoard()` 래퍼 [Wave 1]
  - [ ] 14-02-PLAN.md — iOS 보드 화면: flat 장소 리스트(출처 ▶칩) + 지도 `fitToCoordinates` (boards/[id].tsx) [Wave 2]
  - [ ] 14-03-PLAN.md — iOS 장소 영상 미리보기: `react-native-webview` 임베드 플레이어(PinBottomSheet 확장, start=초) [Wave 2]
  - [ ] 14-04-PLAN.md — iOS 공유 흐름: "친구와 정하기" CTA + 상시 공유 → `shareBoard` → 네이티브 `Share.share(${webUrl}/b/{slug})` [Wave 3, depends 14-01·14-02]
**UI hint:** yes
